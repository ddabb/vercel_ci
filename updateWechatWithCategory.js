...
async function fetchTitleAndCategory(url) {
    try {
        ...
        // 提取分类
        const categories = $('.box.category').map((_, el) => {
            return $(el).text().trim();
        }).get();
        ...
        return { title: null, category: '未分类' };
    }
}

async function updateWechatWithCategory() {
    try {
        // 读取wechat.json文件
        ...
        for (let i = 0; i < wechatData.length; i++) {
            ...
            try {
                const { title, category } = await fetchTitleAndCategory(item.url);
            ...
            // 每处理5个URL后保存一次
            if ((i + 1) % 5 === 0 || i === wechatData.length - 1) {
                console.log('\n保存进度...');
        ...
        await fs.writeFile(jsonPath, JSON.stringify(wechatData, null, 2), 'utf8');
        console.log('\n文件更新完成！');
        console.log(`成功更新: ${successCount} 个条目`);
        console.log(`失败: ${failCount} 个条目`);
